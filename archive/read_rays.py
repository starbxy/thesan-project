...
def read_rays(snap=80, rays_dir='.'):
    ...
    with h5py.File(filename, 'r') as f:
        # print([key for key in f.attrs.keys()])
        # print(f.keys())
        ...
        # Read and perform calculations for each ray
        # Note: The data is float32 so convert to float64 if using cgs units
        ...
        for i in range(n_rays):
            ...
            RM_grid[i] = RM
            # print(f"RM = {RM}")
            # nH_array = np.append(nH_array, n_H)
    ...
    # with h5py.File(filename, 'w') as f:
        # f.attrs['Redshift'] = z
        # f.create_dataset('nH', data=nH_array)
